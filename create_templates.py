...
#
#It turns out that *.index() doesn't work on lists of numpy matrices, so once we have the list of 10,080 templates
#we wont be able to search them (efficiently) using *.index(). As a convenient workaround, after we create the list
#of 10,080 templates, we create a second list, called 'templateHashes' that stores the md5 hashes of the numpy matrices.
#That is templateHashes[i] is the md5 hash of template number i.  This makes for quick & easy searching, which will be
#useful later.
...
def main():
    ...
    #now we generate the list of hashes and pickle them:
    templateHashes = []
    for t in output_list:   templateHashes.append(hashlib.md5(t).hexdigest())
    fileObject = open("templateHashes.p", "wb")
    pickle.dump(templateHashes, fileObject)
    fileObject.close()
