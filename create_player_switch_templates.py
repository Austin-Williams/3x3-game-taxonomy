# The purpose of this program is to create a 362,880 element long list that shows the
...
# Next, put the matrix into standard form, and you're done. The result is the template that
# corresponds to the permutation.

# Once all of these are stored, we'll pickle the result as player_switch_templates.p for later
# use. The output will be a list where the the ith element of the list is the template that
# corresponds to the ith permutation in list(permutations(range(1,10))). The pickled file is
# about 140mb.
...
def main():
    i = 0  # this will keep track of how many permutation/template pairs we've already stored
    ...
    # when this line of code is reached, all the permutation/template pairs have been stored in
    # output_list. We need only store the results.
